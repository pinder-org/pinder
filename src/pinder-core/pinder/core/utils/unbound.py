...
def get_unbound_interface_metrics(
    ...
    bound_contacts = holo_complex.get_contacts(radius=contact_rad)
    ...
    unbound_contacts = apo_RL.get_contacts(radius=contact_rad)
...
def get_unbound_difficulty(
    ...
    apo_R_super, Lrmsd_R, _ = apo_R.superimpose(holo_R)
    apo_L_super, Lrmsd_L, _ = apo_L.superimpose(holo_L)
...
def get_apo_monomer_difficulty(
    ...
    apo_mono_super, Lrmsd, _ = apo_mono.superimpose(holo_ref)
    ...
    fnat_metrics = get_unbound_interface_metrics(
        holo_complex, apo_RL, R_chain, L_chain, holo2apo_seq, apo2holo_seq, contact_rad
    )
